...
```{r}
#Input:
#TDT.dat is a dataframe of static temperature failure data with each row representing one individual organism and columns: 
#"Tref" --> Static trial temperature
#"time" --> time to coma (in minutes)
#event --> 1 for failure, 0 for no failure (right censoring) It is recommended to run all trials until failure so this column only contains "1"s
#temp.dat is fluctuating temperature conditions (temperature vs time) with which survival is predicted
#Each row is a single timepoint 
#Columns:
#Temperature-->temperature at time i
#Time_min--> time i (in minutes)

#Output:
#The function returns a list with the following objects:
#"med.tdt"--> the median predicted failure time if individuals from the same population as the static TDT trial subjects were exposed to the provided fluctuating temperature conditions
#"expected.tdt"--> The expected value, or mean, of the predicted failure distribution under the provided fluctuating temperature conditions
#"cum.surv"--> A vector of the predicted cumulative survival values corresponding to times in temp.dat$Time_min
#"shape.pars" --> A vector of length 3: The first two entries are the slope and intercept of the linear model for Weibull shape parameter vs static temperature. The third entry is the best constant-shape estimate.
#logscale.pars --> A vector of length 2 corresponding to the slope and intercept of the linear model for Weibull scale parameter vs static temperature.
